                    ...
                    <tbody>
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td>
                                    <a
                                    href="{{ url('ViewerJS/#../files/sk/' . $item->link) }}">Lihat</a>
                                </td>
                                <td>
                                    <button style="width: 100px; margin-bottom: 2px" class="btn btn-sm btn-info" onclick="
                                    $('#edit_form').attr('action', '{{ route('panel.sk.update', $item->id) }}')
                                    $('#edit_no').val('{{ $item->no }}');
                                    ...
                                    " class="btn btn-sm btn-danger">
                                        <form method="post" action="{{ route('panel.sk.destroy', $item->id) }}">
                                            @csrf
                                        </form>
                                        Hapus
                                    </button>
...
@section('modals')
    ...
    <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form enctype="multipart/form-data" action="{{ route('panel.sk.store') }}" method="post" class="modal-content">
                @csrf
